...
export const serve = (handle, defaultPort = 8000) => {
  const port = parseInt((program.processedArgs ? program.processedArgs[0] : null) || defaultPort);
  return _serve(async (req, conninfo) => {
    const path = new URL(req.url).pathname;
    return await handle(req, path, conninfo);
  }, { port, hostname });
};

export const serveAPI = (apipath, func, defaultPort = 8000) => {
  server = serve(async (req, path, conninfo) => {
